...
public class RegistroPlaca {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Integer> mapaPlacas = new HashMap<>();
        for (int i = 0; i < placas.length; i++) {
            mapaPlacas.put(placas[i], i + 1);
        }
        
        encontrarMomento(mapaPlacas, "1860");
        encontrarMomento(mapaPlacas, "1440");
        encontrarMomento(mapaPlacas, "1521");
        encontrarMomento(mapaPlacas, "1040");
        encontrarMomento(mapaPlacas, "1225");
        encontrarMomento(mapaPlacas, "1370");
        encontrarMomento(mapaPlacas, "1579");
        encontrarMomento(mapaPlacas, "1901");
        encontrarMomento(mapaPlacas, "1515");
        encontrarMomento(mapaPlacas, "1770");
        encontrarMomento(mapaPlacas, "1620");
        encontrarMomento(mapaPlacas, "1095");
        encontrarMomento(mapaPlacas, "1263");
        encontrarMomento(mapaPlacas, "1105");
    }
    
    public static void encontrarMomento(Map<String, Integer> mapaPlacas, String placa) {
        if (mapaPlacas.containsKey(placa)) {
        	LocalDateTime data = LocalDateTime.now();
        	DateTimeFormatter dataFormatada = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        	String dataHoje = data.format(dataFormatada);
            System.out.println("* Registro: " + dataHoje);
            System.out.println("* Placa: " + placa);
            System.out.println("* Momento (posição) na via: " + mapaPlacas.get(placa));
            System.out.println();
        } else {
